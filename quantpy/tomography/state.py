...
class StateTomograph:
    ...
    def mhmc(self, n_points, step=0.01, burn_steps=1000, thinning=1, warm_start=False,
        ...
        if not (warm_start and hasattr(self, 'chain')):
            ...
            self.chain = MHMC(lambda x: -self._nll(x), step=step, burn_steps=burn_steps, dim=dim,
        ...
        dist = np.asarray(
            [self.dst(_real_tril_vec_to_matrix(tril_vec), state.matrix) for tril_vec in samples])
        dist.sort()
    ...
    def bootstrap(self, n_points, method='lin', physical=True, init='lin', tol=1e-3, max_iter=100,
        ...
        if not use_new_estimate:
            ...
            state = self.point_estimate(method=method, physical=physical, init=init, tol=tol,
                                        max_iter=max_iter)
    ...
    def sugiyama_interval(self, n_points=1000, dist=None):
        ...
        POVM_matrix = np.reshape(self.POVM_matrix,
                                 (-1, self.POVM_matrix.shape[-1])) * 2 ** self.state.n_qubits
        inversed_POVM = _left_inv(POVM_matrix).reshape(
            (-1, self.POVM_matrix.shape[0], self.POVM_matrix.shape[1]))
        ...
        CLs = 1 - 2 * np.sum(
            np.exp(-b * dist[:, None] ** 2 * np.sum(self.n_measurements) / c_alpha[None, :]),
            axis=1)
        return dist, CLs

    def wang_interval(self, n_points, mode='coarse'):
        ...
        if mode == 'coarse':
            ...
            coef1 = np.linalg.norm(A, ord=2) * (self.POVM_matrix.shape[1] - 1) * np.sqrt(prob_dim)
